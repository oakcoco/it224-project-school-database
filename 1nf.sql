...
-- ============================================================================
-- FIRST NORMAL FORM (1NF) - schema.sql
-- ============================================================================
-- Rules:
...
-- The current schema.sql is ALREADY in 1NF because:
-- - Each column has atomic values (no lists/arrays)
-- - All tables have PRIMARY KEY
-- - No repeating groups exist

-- However, we identify potential issues:

-- ISSUE 1: grade_year column in grades table
-- The comment says "grade year will be deleted when altered"
-- This suggests grade_year might be a temporary/derived field

-- ISSUE 2: ENUM types define allowed values (already atomic - OK)

-- ISSUE 3: Foreign keys properly link tables (no duplicate data in cells)


-- ============================================================================
-- 1NF RULES APPLIED TO CURRENT SCHEMA:
-- ============================================================================

-- Table: institute (1NF Compliant)
CREATE TABLE IF NOT EXISTS institute (
    institute_id INT AUTO_INCREMENT PRIMARY KEY,
    institute_name VARCHAR(100) NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    -- Atomic values: single institute_name per row
    -- Unique: institute_id primary key
);

-- Table: course (1NF Compliant)
CREATE TABLE IF NOT EXISTS course (
    ...
    -- Atomic values
    -- Unique via primary key
);

-- Table: section (1NF Compliant)
...
CREATE TABLE IF NOT EXISTS students (
    ...
    -- All atomic values
    -- Note: age could be derived from date_of_birth (2NF issue)
);

-- Table: grades (1NF Compliant but noted issue)
CREATE TABLE IF NOT EXISTS grades (
    ...
    -- grade_year: atomic integer value
    -- Unique constraint ensures no duplicate grade records
);

-- ============================================================================
-- SUMMARY: Current schema.sql is already in 1NF
-- ============================================================================
-- No changes needed - all values are atomic, primary keys defined


-- ============================================================================
-- INSERT DATA FOR 1NF TABLES
-- ============================================================================
-- These inserts demonstrate 1NF: atomic values, unique rows, no repeating groups
...
-- Insert Section data (needed for students)
...
-- Insert Student data (each row has atomic values, no repeating groups)
...
-- Insert Subject data (needed for grades)
...
-- Insert Grade data (each grade is a SEPARATE row - NO repeating groups!)
-- In UNF, grades would be in repeating columns. In 1NF, each grade = 1 row
INSERT INTO grades (student_id, subject_id, grade) VALUES
-- Student 1 (Mohammed, course CS) has 2 grades - each in SEPARATE row
(1, 1, 1.50),
(1, 2, 1.75),
-- Student 2 (Kassandra, course MSAI) has 2 grades - each in SEPARATE row
(2, 3, 2.75),
(2, 4, 1.25),
-- Student 3 (Juan, course CE) has 1 grade
(3, 5, 2.25),
-- Student 4 (Ana, course BA) has 1 grade
(4, 6, 1.25);

-- ============================================================================
-- 1NF DEMONSTRATED IN THIS INSERT:
-- ============================================================================
...
-- 4. SAME TYPE: grade column is always DECIMAL(3,2), grade_year is always INT
--
-- Compare to UNF: In UNF, student 1's grades would be in columns like grade1, grade2
-- In 1NF: Each grade is its own row with unique primary key